# Fork tracking — intarweb/docker-autoheal

This is a soft fork of [`willfarrell/docker-autoheal`](https://github.com/willfarrell/docker-autoheal). We track upstream HEAD and publish a HEAD-tracked image to GHCR with any open intarweb→upstream PRs cherry-picked on top.
...
| Property | Value |
|---|---|
| Upstream | [`willfarrell/docker-autoheal`](https://github.com/willfarrell/docker-autoheal) |
...
| Property | Value |
|---|---|
| Image | `ghcr.io/intarweb/autoheal` |
...
services:
  autoheal:
    image: ghcr.io/intarweb/autoheal:latest
```

Pin to a specific build for reproducibility:

```yaml
    image: ghcr.io/intarweb/autoheal:sha-<short>
```
...
```bash
gh workflow run "Sync from upstream + auto-regen intarweb-dev" --repo intarweb/docker-autoheal
```

**Force a fresh build of `:latest` without an upstream change:**
```bash
gh workflow run "Build from source → GHCR" --repo intarweb/docker-autoheal --ref intarweb-dev
```
...
gh pr create --repo willfarrell/docker-autoheal --base main --head intarweb:feat/my-thing
